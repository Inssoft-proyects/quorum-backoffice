...
/** Returns the implicit ARIA role for a tag, or undefined. */
function implicitRole(el: Element): string | undefined {
  ...
  switch (tag) {
    ...
    case 'input': {
      ...
      if (type === 'email' || type === 'url' || type === 'tel') return 'textbox';
...
function collectText(el: Element): string {
  // Walk children for text content. aria-label/aria-labelledby win first.
  ...
  // <svg>: try aria-label / title child
  if (el.tagName === 'svg' || el.tagName === 'SVG') {
    const title = el.querySelector('title');
    if (title?.textContent) return title.textContent.trim();
  }
  // Otherwise: text content of the element itself (not deep).
