...
| Polish WU v6 (Auth OTP) | ✅ done | `2d03fde` + `b66a7df` + `88d92b4` + `ceeef88` + `74d43f9` + `8f46a39` + `7fb2a22` + `cd96e99` | ~1640 (across 8 commits) | Security rewrite: migrate login from `email + password` to `email + OTP`. **(A1)** `feat(api): add SMTP mailer + config` — Nodemailer + dev-mode logged fallback. **(A2)** `feat(api): OtpClient.issue()` — extends verify() so backoffice requests OTP from quorum-otp service. **(A5)** `feat(auth): DTOs + client wiring` — `RequestLoginRequest`/`RequestLoginResponse` + `LoginRequestOtp` (password kept as ignored optional field for back-compat). **(A3+A4)** `feat(api): email+OTP login flow` — `AuthService.requestLoginOtp()` (idempotent 200 for unknown emails) + `loginWithOtp()` (OTP-service verify + session issue), routes `POST /api/v1/auth/login/request` + `POST /api/v1/auth/login`, plugin `auth-deps` registering OtpClient+Mailer at boot, migration `0010_audit_action_otp_login.sql` extending `audit_action` enum with `auth.login.requested`/`auth.login.otp_verified`/`auth.login.failed`. **(A6)** `test(api): migrate integration suite` — auth.test.ts (21 tests), rbac.test.ts, marbetes.test.ts, migrations.test.ts all updated to drive the 2-step flow with `FakeOtpClient` + `createMailerForTest` injection. **(A7)** `feat(web): 2-step LoginFormOtp` — composed from shadcn primitives (Input, Label, Button, Alert) + existing `OtpInput`; deleted legacy `LoginForm`. **(A8)** `test(web): RTL coverage` — 6 new tests in `login-form-otp.test.tsx`; updated `auth-context.test.tsx` for the new contract. **(A9)** `test(e2e): Playwright spec` — 6 cases (T10.1-T10.6) including a regression guard asserting NO `<input type="password">` in the login DOM (closes the original "credenciales en URL" finding). Result: **205/205 tests verde** (122 API + 83 web). `users.password_hash` preserved on disk but ignored in the login flow (legacy recovery). RUNBOOK updated with new `SMTP_*` + `LOGIN_OTP_*` env vars + new "OTP delivery" section. Plan + bitácora en `odd/tasks/security-auth-otp-and-maquette-v3.md`. |
| --- | --- | --- | --- | --- |
