...
| WU8b1 | ✅ done | — | ~370 | API canvasUserId + students endpoint + inactive rejection. Migration 0006 (is_active BOOLEAN en students_cache), DTO rename (assignedStudentId → canvasUserId), GET /api/v1/students?canvasUserId=X con requireRole('admin'), marbetes service resuelve canvas_user_id → students_cache.id y rechaza 422 si no existe o inactivo, marbetes integration tests actualizados (7 nuevos en describe block canvas_user_id). 13 nuevos tests (85/85 API total verde, 28/28 web sin regresiones). WU8b2 = web create+edit dialogs usando este contrato. |
| WU8b2 | pending | — | — | Web marbetes create + edit dialogs usando canvasUserId (lookup inline contra /students antes de submit). |
