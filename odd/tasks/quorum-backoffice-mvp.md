...
| Polish WU v6 (Auth OTP + maquette v3) | ✅ done | `2d03fde` + `b66a7df` + `88d92b4` + `ceeef88` + `74d43f9` + `8f46a39` + `7fb2a22` + `cd96e99` + `75e5e01` + `783b350` + `47a4f6a` + `7fe86d4` | ~1800 (14 commits) | **Fase A — Security rewrite**: migrate login from `email + password` to `email + OTP`. (A1) `2d03fde feat(api): add SMTP mailer + config` — Nodemailer + dev-mode logged fallback. (A2) `b66a7df feat(api): OtpClient.issue()` — extends verify(). (A5) `88d92b4 feat(auth): DTOs + client wiring` — `RequestLoginRequest`/`LoginRequestOtp` (password kept as ignored optional). (A3+A4) `ceeef88 feat(api): email+OTP login flow` — `AuthService.requestLoginOtp()` (idempotent 200) + `loginWithOtp()`, routes `POST /auth/login/request` + `POST /auth/login`, plugin `auth-deps`, migration `0010_audit_action_otp_login.sql`. (A6) `74d43f9 test(api): migrate integration suite` — auth.test.ts (21 tests), rbac.test.ts, marbetes.test.ts, migrations.test.ts with `FakeOtpClient` + `createMailerForTest`. (A7) `8f46a39 feat(web): 2-step LoginFormOtp` — composed from shadcn primitives + existing `OtpInput`; deleted legacy `LoginForm`. (A8) `7fb2a22 test(web): RTL coverage` — 6 new tests in `login-form-otp.test.tsx`. (A9) `cd96e99 test(e2e): Playwright spec` — 6 cases (T10.1-T10.6) with regression guard `expect(input[type="password"]).toHaveCount(0)`. **Fase B — Maquette v3 alignment**: (B1) `75e5e01 chore: drop __MACOSX + png-x2 leftovers` + `783b350 chore: regenerate package-lock`. (B7) `47a4f6a test(e2e): fix T10.3 + add mock-otp-service for local Playwright runs` + `7fe86d4 test(e2e): maquette v3 visual regression` — `11-maquette-v3.spec.ts` (T11.1-T11.5) verifica que `/marbetes` + `/dispositivos` + `/audit` + `/dashboard` + `/login` renderizan con el copy y métricas del HTML canónico; screenshots en `apps/web/e2e/lookfeel/artifacts/`. **Total: 216/216 tests verde** (122 API + 83 web + 11 Playwright). PR: https://github.com/Inssoft-proyects/quorum-backoffice/pull/3 |
| --- | --- | --- | --- | --- |
