...
| WU6b | ✅ done | — | ~430 | API Session plugin + RBAC + wire. `session-hydrator.ts` (token→user) + `plugins/session.ts` (global onRequest hook con x-test-actor shim + cookie lookup) + `plugins/rbac.ts` (requireSession + requireRole factories) + preHandlers en marbetes/dispositivos/audit (`requireSession` en GET, `requireRole('admin')` en POST/PATCH/DELETE, `requireRole('auditor')` en GET audit) + `actorFromRequest` ahora lee de `req.session.user.email`. 16 nuevos tests integration (72/72 total verde). x-test-actor shim mantenido para compatibilidad con tests existentes. |
