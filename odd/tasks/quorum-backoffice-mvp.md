...
| WU12 | ✅ done | — | ~300 | Infra: nginx reverse proxy + systemd units + CORS_ORIGIN env. Vhost `infra/nginx/quorum.asistentepro.mx.conf` con TLS 1.2/1.3, security headers (HSTS, X-Frame-Options DENY, Permissions-Policy, etc.), rutas /healthz|/readyz|/metrics|/api/|web + catch-all Next.js, _next/static cache 1yr. 2 systemd templates (api+web) con hardening (NoNewPrivileges, ProtectSystem=strict, ReadWritePaths=dist). api.config: ALLOWED_ORIGIN env var wired a cors plugin (optional, defaults a false para dev). web/env.example: NEXT_PUBLIC_API_URL apunta a `https://quorum.asistentepro.mx`. RUNBOOK + infra/nginx/README.md con full operational guide (install, certbot, env files, systemd, logs, update, rollback). |
